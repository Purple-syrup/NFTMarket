    ...
    : describe("NFT Marktplace Unit Tests", () => {
          ...
          describe("listItem", () => {
              it("should revert if marketplace not approved", async () => {
                  await expect(
                      nftMarketplace.listItem(mockNft.address, 0, price)
                  ).to.be.revertedWith("NftMarketplace__NotApprovedForMarketPlace")
              })
              it("to emit ItemListed", async () => {
                  await mockNft.approve(nftMarketplace.address, 0)
                  await expect(nftMarketplace.listItem(mockNft.address, 0, price)).to.emit(
                      nftMarketplace,
                      "ItemListed"
                  )
              })
          ...
          describe("buyItem", () => {
              ...
              it("should fail if not a listed nft", async () => {
                  await expect(nftMarketplace.buyItem(mockNft.address, 2, { value: price }))
                      .to.be.revertedWith(`NftMarketplace__NotListed`)
                      .withArgs(mockNft.address, 2)
              })

              it("should fail if value is less then price", async () => {
                  await expect(
                      nftMarketplace.buyItem(mockNft.address, 0, { value: "100000000" })
                  ).to.be.revertedWith("NftMarketplace__PriceNotMet")
              })

              it("should emit Item bought if successfull ", async () => {
                  await expect(
                      nftMarketplace.connect(player).buyItem(mockNft.address, 0, { value: price })
          ...
          describe("cancelListing", () => {
              ...
              it("should revert if not owner", async () => {
                  await expect(
                      nftMarketplace.connect(player).cancelListing(mockNft.address, 0)
                  ).to.be.revertedWith("NftMarketplace__NotOwner")
              })

              it("should delete listing when cancelled by owner", async () => {
                  await expect(nftMarketplace.cancelListing(mockNft.address, 0)).to.emit(
                      nftMarketplace,
                      "ItemCancelled"
                  )
